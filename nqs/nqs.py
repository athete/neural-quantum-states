...
class NeuralQuantumState:
    ...
    A NQS is essentially a restricted Boltzmann machine (RBM) consisting of a visible layer (corresponing to the spins)
    and a hidden layer. 
    '''
    def __init__(self, numHidden=40, numVisible=40, initWeight=0.001) -> None:
        ...
        # Network weights
        self.W = (np.random.rand(self.numVisible, self.numHidden) - 0.5) * initWeight 
    ...
    def initLookupTables(self, state) -> None:
        '''
        Initializes the lookup tables. Given a state, we update the list of theta values
        used to compute the prefactor of a state. 
        '''

        state = np.array(state).astype(float).reshape((self.numVisible, 1))
        self.theta = self.b + np.dot(self.W.T, state)

    def updateLookupTables(self, state, flips) -> None:
        '''
        Updates the lookup tables after a spin flip in the Metropolis-Hastings algorithm
        '''
